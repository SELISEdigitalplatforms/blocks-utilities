...
export default function MagicUrlDetailsPage() {
  ...
  const { data: magicUrl, isLoading } = useGetMagicUrlById({ ItemId: id!, projectKey: tenantId });
  const { deactivateMagicUrl, isRemoving } = useDeactivateMagicUrl();
  const [isDeactivateModalOpen, setIsDeactivateModalOpen] = useState(false);

  BREADCRUMB_CUSTOM_TITLES["/magic-url"] = "Magic URL";
  if (id) {
    BREADCRUMB_CUSTOM_TITLES[`/magic-url/details/${id}`] = magicUrl?.name || "Details";
  }
        ...
        <Card>
              ...
              <Dialog open={isDeactivateModalOpen} onOpenChange={setIsDeactivateModalOpen}>
                ...
                <ConfirmationModal
                  ...
                  data={{
                    ...
                    confirmButton: isRemoving ? "Deactivating..." : "Deactivate",
                    cancelButton: "Cancel",
                  }}
          ...
          <CardContent className="flex flex-col gap-6">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
              <div>
                <p className="mb-2 text-sm text-muted-foreground">Total Usage</p>
                <div className="flex items-center gap-3">
                  <span className="text-lg font-medium">
                    {magicUrl.usageCount} / {magicUrl.usageLimit === 0 ? "Unlimited" : magicUrl.usageLimit}
                  </span>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
              <div>
                ...
                <p className="text-base">{formatDate(parseDateString(magicUrl.createdAt))}</p>
              </div>
              <div>
                <p className="mb-2 text-sm text-muted-foreground">Scheduled Expiry Date</p>
                <p className="text-base">
                  {magicUrl.expiryDate ? formatDate(parseDateString(magicUrl.expiryDate)) : "-"}
                </p>
              </div>
              <div>
                <p className="mb-2 text-sm text-muted-foreground">Expiry Reason</p>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div>
                <p className="mb-2 text-sm text-muted-foreground">Shortened URL</p>
                <CopyToClipboardButton textToCopy={magicUrl.shortUri}>
                  <span className="max-w-[400px] truncate font-mono text-sm">{magicUrl.shortUri}</span>
                </CopyToClipboardButton>
              </div>
              <div>
                <p className="mb-2 text-sm text-muted-foreground">Original URL</p>
                <CopyToClipboardButton textToCopy={magicUrl.uri}>
                  <span className="max-w-[400px] truncate font-mono text-sm">{magicUrl.uri}</span>
                </CopyToClipboardButton>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
              <div>
                <p className="mb-2 text-sm text-muted-foreground">Request Method</p>
                <p className="text-base uppercase">{magicUrl.requestMethod || "-"}</p>
              </div>
              <div>
                <p className="mb-2 text-sm text-muted-foreground">Client Credential</p>
                <p className="max-w-[200px] truncate text-base">{magicUrl.clientCredential || "-"}</p>
              </div>
              <div>
