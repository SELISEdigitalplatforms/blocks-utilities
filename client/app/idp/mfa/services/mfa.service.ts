...
export class MFAService {
  getConfigurations(payload: IGetConfigurationPayload): Promise<IGetConfigurationResponse> {
    return serviceInstances.idpService.get(`${MFA_CONFIG_ENDPOINTS.GET}?ProjectKey=${payload.projectKey}`, undefined, { absoluteUrl: true });
  }

  saveMFAConfiguration(
    payload: IMFAConfigurationSavePayload,
  ): Promise<IMFAConfigurationSaveResponse> {
    return serviceInstances.idpService.post(MFA_CONFIG_ENDPOINTS.SAVE, payload, undefined, { absoluteUrl: true });
  }

  generateUserMfaOTP(payload: IGenerateUserMFA_OtpPayload): Promise<IGenerateUserMFA_OtpResponse> {
    return serviceInstances.idpService.post(MFA_ENDPOINTS.GENERATE_OTP, payload, undefined, { absoluteUrl: true });
  }

  configureUserMFA(payload: IConfigureUserMFAPayload): Promise<IConfigureUserMFAResponse> {
    return serviceInstances.idpService.post(MFA_ENDPOINTS.CONFIGURE_USER_MFA, payload, undefined, { absoluteUrl: true });
  }
  ...
  verifyOtp(payload: IVerifyMfaOtpPayload): Promise<IVerifyMfaOtpResponse> {
    return serviceInstances.idpService.post(MFA_ENDPOINTS.VERIFY_OTP, payload, undefined, { absoluteUrl: true });
  }

  resendOtp(payload: IResendMfaOtpPayload): Promise<IVerifyMfaOtpResponse> {
    return serviceInstances.idpService.post(MFA_ENDPOINTS.RESEND_OTP, payload.mfaId, undefined, { absoluteUrl: true });
  }
  disableMFA(payload: IDisableMFAPayload): Promise<IDisableMFAResponse> {
    return serviceInstances.idpService.post(MFA_ENDPOINTS.DISABLE_MFA, payload, undefined, { absoluteUrl: true });
  }
}
