...
const getStatus = (item: MagicUrl) => {
  // If the API returns a status directly, map it or use it.
  if (item.status) {
    switch (item.status) {
      case "Active":
      ...
      case "Inactive":
      case "Disabled":
        return { label: item.status, variant: "error" }; // Or secondary depending on design
      case "Expired":
        return { label: "Expired", variant: "error" };
    }
  }

  // Fallback logic for legacy or if status is not provided
...
export function MagicUrlStatusBadge({ item }: { item: MagicUrl }) {
  ...
  return (
    <Badge variant={variant as any} className="whitespace-nowrap rounded-full">
      {label}
    </Badge>
  );
}
