import { z } from "zod";
import { INotificationConfig } from "@blocks-utilities/notification/models/notification.model";
import { useSaveNotificationConfig } from "@blocks-utilities/notification/hooks/use-notifications";
...
const NewNotificationConfiguration: React.FC<NewNotificationConfigurationProps> = ({
    ...
    try {
      ...
      if (res?.isSuccess) {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: res?.errors ? JSON.stringify(res.errors) : "Failed to save configuration",
        });
      }
    } catch (error) {
      toast({
  ...
  return (
    <DialogContent className="rounded-md sm:max-w-[700px]" data-testid={dataTestId}>
      <Form {...form}>
