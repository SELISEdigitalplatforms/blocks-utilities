...
public static class SubscriptionDocumentSourceFactory
{
    ...
    /// <summary>
    /// The obligation a change that banks credit leaves behind, figures and all.
    /// </summary>
    /// <remarks>
    /// The one source that must carry its own amounts, because it is the only record of them: the
    /// change charges nothing, so there is no payment to read the figures off, and the balance it
    /// moved cannot say which change moved it. This is also the only source that must be appended in
    /// the same write as the transition — anywhere else, a crash between the two loses the credit note
    /// permanently.
    /// </remarks>
    /// <param name="subscription">
    /// The subscription <em>before</em> the change is applied. The credit is unused time on the plan
    /// being left, so that is the plan the document has to name.
    /// </param>
    public static SubscriptionDocumentSource? ForBankedCredit(
        SubscriptionDetail subscription,
        ...
        string correlationId)
    {
        ...
        return new SubscriptionDocumentSource
        {
            ...
            [
                ...
                {
                    Description =
                        $"Unused time credited from {subscription.Plan.DisplayName}",
